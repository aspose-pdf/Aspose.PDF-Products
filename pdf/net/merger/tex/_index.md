...
+  Load all the TEX files with full path.
+  Make one document as the base file
+  Call the relevant method for concatenating and merging files one by one.
+  Call the Save() method and pass the file name (full path) and format (TEX) as a parameter.
+  Now you can open and use the TEX file in Microsoft Office, Adobe PDF or any other compatible program.
...
```cs
    // Open first document
    Document file1 = new Document("file1.tex");
    // Open second document
    Document file2 = new Document("file2.tex");
    // Add pages of second document to the first
    file1.Pages.Add(file2.Pages);
    // Save concatenated output file
    file1.Save("ConcatenatedFiles_out.tex");  
